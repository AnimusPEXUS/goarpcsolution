module github.com/AnimusPEXUS/gorpcsolution

go 1.19

require (
	github.com/AnimusPEXUS/gojsonrpc2 v0.0.0-20230604165602-032924975eaf
	github.com/AnimusPEXUS/golockerreentrancycontext v0.0.0-20230205202617-6e6a53c419ed
	github.com/AnimusPEXUS/gouuidtools v0.0.0-20230604162424-bdda2f021c7d
	github.com/AnimusPEXUS/utils v0.0.0-20230429235807-5de56d5918ef
	github.com/mitchellh/mapstructure v1.5.0
)
