...
func (self *ARPCSolutionNode) PushMessageFromOutside(
	...
	if !ok {
		return errors.New("can't convert msg.Params to map[string]string"),
	...
	if !msg_has_id {
		...
		case "NewCall":
			...
			if err != nil {
				return errors.New("new call id required"),
					errors.New("protocol error")
			}
		...
		default:
			return errors.New("invalid method name"),
				errors.New("protocol error")
		case "CallGetList":
			result, err_reply, err = self.controller.CallGetList()
		case "CallGetArgCount":
			...
			if err_reply != nil || err != nil {
				break
			}

			result, err_reply, err = self.controller.CallGetArgCount(
		...
		case "CallGetArgValue":
			...
			result, err_reply, err = self.controller.CallGetArgValue(
				uuid.FromStringOrNil(call_id_str),
				arg_index,
			)
		case "BufferGetInfo":
